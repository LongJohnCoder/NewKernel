  ...
  )
{
  ...
  if (mHiiPackageListUpdated) {
    ...
    //
    DMSG ((EFI_D_ERROR, "\n UpdateStatement, Hii Package is Updated.\n"));
    mHiiPackageListUpdated = FALSE;
  ...
  )
{
  ...
  if ((!IsZeroGuid (&Statement->RefreshGuid)) || (Statement->RefreshInterval != 0)) {
	...
	DMSG ((EFI_D_ERROR, "InitializeDisplayStatement, RefreshInterval :%d. \n", Statement->RefreshInterval));

    gDisplayFormData.FormRefreshEvent = mValueChangedEvent;

    DisplayStatement->RefreshInterval = Statement->RefreshInterval;
  }
...
/**
  Update Displaying statement to avoid exit.

  @param Statement           The statement need to check.

**/
VOID
SyncDisplayStatement (
  IN FORM_BROWSER_STATEMENT        *Statement
  )
{
  FORM_DISPLAY_ENGINE_STATEMENT *DisplayStatement;
  ...
  while (!IsNull (&gDisplayFormData.StatementListHead, Link)) {
    DisplayStatement = FORM_DISPLAY_ENGINE_STATEMENT_FROM_LINK (Link);
    if (DisplayStatement->OpCode == Statement->OpCode) {
      DMSG ((EFI_D_ERROR, "SyncDisplayStatement, OpCode :0x%x. \n", DisplayStatement->OpCode));
		
      CopyMem (&DisplayStatement->CurrentValue, &Statement->HiiValue, sizeof (EFI_HII_VALUE));	  
      if (Statement->BufferValue != NULL) {
        CopyMem (DisplayStatement->CurrentValue.Buffer, Statement->BufferValue, DisplayStatement->CurrentValue.BufferLen);
      }
    }
    Link = GetNextNode (&gDisplayFormData.StatementListHead, Link);
  }
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "\n RefreshIntervalProcess, RefreshItemFlag :%d.\n", gDisplayFormData.RefreshItemFlag));
  ...
  while (!IsNull (&gCurrentSelection->Form->StatementListHead, Link)) {
    ...
    DMSG_HII(Statement->Prompt, gCurrentSelection->Handle, L"Statement");
    UpdateStatement(Statement);

    if (gDisplayFormData.RefreshItemFlag == 0) {
      SyncDisplayStatement (Statement);
    }	
  }
  ...
  )
{
  ...
  while (!IsNull (&gBrowserHotKeyList, Link)) {
    ...
    DMSG ((EFI_D_ERROR, "CopyKey :0x%x.\n", CopyKey->KeyData->ScanCode));
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "\n AddStatementToDisplayForm, Begin, \n"));
  //
  ...
  InsertTailList(&gDisplayFormData.StatementListOSF, &DisplayStatement->DisplayLink);
  DMSG ((EFI_D_ERROR, "AddStatementToDisplayForm, StatementListOSF, \n"));
  ...
  while (!IsNull (&gCurrentSelection->Form->StatementListHead, Link)) {
    ...
    //DMSG_HII(Statement->Prompt, FormSet->HiiHandle, L"Statement");	
    DisplayStatement = AllocateZeroPool (sizeof (FORM_DISPLAY_ENGINE_STATEMENT));
    ASSERT (DisplayStatement != NULL);
  ...
  if (MinRefreshInterval != 0) {
    DMSG ((EFI_D_ERROR, "AddStatementToDisplayForm, MinRefreshInterval :%d. \n", MinRefreshInterval));
    ...
    ASSERT_EFI_ERROR (Status);
    DMSG ((EFI_D_ERROR, "AddStatementToDisplayForm, RefreshIntervalEvent :0x%x. \n", RefreshIntervalEvent));
  ...
  DMSG ((EFI_D_ERROR, "AddStatementToDisplayForm, End. \n"));
}
  ...
  )
{
  ...
  ASSERT_EFI_ERROR (Status); 
  DMSG ((EFI_D_ERROR, "\n InitializeDisplayFormData, mValueChangedEvent :0x%x.\n", mValueChangedEvent));
}
  ...
  )
{
  ...
  gDisplayFormData.FormSetTitle = gCurrentSelection->FormSet->FormSetTitle;
  ...
  AddStatementToDisplayForm ();
  DMSG ((EFI_D_ERROR, "UpdateDisplayFormData, End.\n "));
}
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "\n UpdateStatementStatus, SettingScope :%d. \n", SettingScope));
  switch (SettingScope) {
  case SystemLevel:
    Link = GetFirstNode (&gBrowserFormSetList);
    while (!IsNull (&gBrowserFormSetList, Link)) {
  ...
  )
{
  ...
  if ((Action & BROWSER_ACTION_RESET) == BROWSER_ACTION_RESET) {
    gResetRequired = TRUE;
    DMSG ((EFI_D_ERROR, "ProcessAction, Set Reset Flag. \n"));
  }
  ...
  if ((Action & BROWSER_ACTION_MOVE_UP) == BROWSER_ACTION_MOVE_UP) {
    gItemMoveAction = BROWSER_ACTION_MOVE_UP;
  }  

  if ((Action & BROWSER_ACTION_MOVE_DOWN) == BROWSER_ACTION_MOVE_DOWN) {
    gItemMoveAction = BROWSER_ACTION_MOVE_DOWN;
  }  
  ...
  )
{
  ...
  RetValue = TRUE;
  switch (mFormDisplay->ConfirmDataChange(NULL)) {
  ...
  )
{
  ...
  if (UserInput->Action != 0) {
    ...
    if (UserInput->Action != BROWSER_ACTION_MOVE_DOWN && UserInput->Action != BROWSER_ACTION_MOVE_UP) {
      gCurrentSelection->Statement = NULL;
    }
  } else {
    ASSERT (Statement != NULL);
    gCurrentSelection->Statement = Statement;
    DMSG ((EFI_D_ERROR, "ProcessUserInput, Operand :0x%x. \n", Statement->Operand));
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "DisplayForm, EvaluateFormExpressions :%r.\n", Status));
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  DMSG ((EFI_D_ERROR, "DisplayForm, FormDisplay :%r.\n", Status));
  if (EFI_ERROR (Status)) {
    FreeDisplayFormData();
    return Status;
  }

  if (gDisplayFormData.RefreshItemFlag == 2) {
    UserInput.Action = 0;
    gDisplayFormData.RefreshItemFlag  = 0;
  }
  
  Status = ProcessUserInput (&UserInput);
  DMSG ((EFI_D_ERROR, "DisplayForm, ProcessUserInput :%r.\n", Status));
  FreeDisplayFormData();
  return Status;
}
  ...
  )
{
  ...
  if (!FormSet->QuestionInited) {
    //DMSG ((EFI_D_ERROR, "\n IsNvUpdateRequiredForFormSet, QuestionInited :%d. \n", FormSet->QuestionInited));
    return FALSE;
  }
  ...
  while (!IsNull (&FormSet->FormListHead, Link)) {
    ...
    if (RetVal) {
      //DMSG ((EFI_D_ERROR, "IsNvUpdateRequiredForFormSet :%d. \n", RetVal));
      break;
    }
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "ReconnectController, \n"));
  ...
  )
{
  ...
  switch (Action) {
    case EFI_BROWSER_ACTION_CHANGING:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Changing. \n", Action));
      break;
    case EFI_BROWSER_ACTION_CHANGED:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Changed. \n", Action));
      break;
    case EFI_BROWSER_ACTION_RETRIEVE:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Retrieve. \n", Action));
      break;
    case EFI_BROWSER_ACTION_FORM_OPEN:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Open. \n", Action));
      break;
    case EFI_BROWSER_ACTION_FORM_CLOSE:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Close. \n", Action));
      break;
    case EFI_BROWSER_ACTION_SUBMITTED:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Submitted. \n", Action));
      break;
    case EFI_BROWSER_ACTION_DEFAULT_STANDARD:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Deault Standed. \n", Action));
      break;
    case EFI_BROWSER_ACTION_DEFAULT_MANUFACTURING:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Default Manufacturing. \n", Action));
      break;
    case EFI_BROWSER_ACTION_DEFAULT_SAFE:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Default Safe. \n", Action));
      break;
    case EFI_BROWSER_ACTION_DEFAULT_PLATFORM:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Default Platform. \n", Action));
      break;
    case EFI_BROWSER_ACTION_DEFAULT_HARDWARE:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Default Hardware. \n", Action));
      break;
    case EFI_BROWSER_ACTION_DEFAULT_FIRMWARE:
      DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction,  Action :0x%x-Default Firmware. \n", Action));
      break;
    default:
      break;
  }
  ...
  )
{
  ...
  #ifdef SETUP_DEBUG
  PrintCallbackActionInfo(Action);
  #endif 
  ...
  while (!IsNull (&Form->StatementListHead, Link)) {
    ...
    DMSG ((EFI_D_ERROR, "\n ProcessCallBackFunction, HiiValue->Type :0x%x.\n", HiiValue->Type));	
    DMSG_HII(Statement->Prompt, FormSet->HiiHandle, L"Statement");
    ...
    DMSG ((EFI_D_ERROR, "ProcessCallBackFunction,  ActionRequest :0x%x, Callback :%r. \n", ActionRequest, Status));
    if (!EFI_ERROR (Status)) {      
      //
      ...
      //
      // EFI_IFR_TYPE_ACTION need to handle reset ActionRequest.
      //
      if (HiiValue->Type == EFI_IFR_TYPE_ACTION && Action == EFI_BROWSER_ACTION_CHANGING) {
        if (ActionRequest == EFI_BROWSER_ACTION_REQUEST_RESET) {
          DMSG ((EFI_D_ERROR, "ProcessCallBackFunction, EFI_IFR_TYPE_ACTION, Set Reset Flag. \n"));
          gResetRequired = TRUE;
        }
      }
      ...
      case EFI_BROWSER_ACTION_CHANGED:
        ...
        case EFI_BROWSER_ACTION_REQUEST_RESET:
          ...
          gResetRequired = TRUE;
          NeedExit              = TRUE;
          DMSG ((EFI_D_ERROR, "ProcessCallBackFunction,  Set Reset Flag. \n"));
          break;
  ...
  DMSG ((EFI_D_ERROR, "ProcessCallBackFunction,  After Callback, \n"));
  ...
  if (NeedExit) {
    if (NULL != gDisplayFormData.ByoCurrentFormSetLink) {
      ByoFormSet = BYO_FORM_BROWSER_FORMSET_FROM_LINK (gDisplayFormData.ByoCurrentFormSetLink);        
      if ((ByoFormSet->FirstFormId == gDisplayFormData.FormId ) && IsByoFormset(gDisplayFormData.ByoFormSetList, gDisplayFormData.HiiHandle)) {
        gExitRequired = TRUE;
        DMSG ((EFI_D_ERROR, "ProcessCallBackFunction,  Set Exit Flag. \n"));
      } else {
  ...
  DMSG ((EFI_D_ERROR, "ProcessCallBackFunction,  End :%r. \n", Status));
  return Status;
}
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "SetupBrowser, RegisterPackageNotify :%r.\n", Status));
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  //
  DMSG ((EFI_D_ERROR, "SetupBrowser, InitializeCurrentSetting, \n"));
  ...
  do {
    ...
    //
    DMSG ((EFI_D_ERROR, "\n SetupBrowser, Begin, mHiiPackageListUpdated :%d.\n", mHiiPackageListUpdated));
    gItemMoveAction = 0;
    if (mHiiPackageListUpdated) {
      DMSG ((EFI_D_ERROR, "\n SetupBrowser, Hii Package is Updated.\n"));
      Selection->Action = UI_ACTION_REFRESH_FORMSET;
      mHiiPackageListUpdated = FALSE;
      break;
    }
    ...
    if (((Selection->Handle != mCurrentHiiHandle) ||
      ...
      if (ConfigAccess != NULL) {
        ...
        DMSG ((EFI_D_ERROR, "SetupBrowser, ProcessCallBackFunction(Open) :%r.\n", Status));		
        if (EFI_ERROR (Status)) {
          goto Done;
        }
        ...
        //
        if (mHiiPackageListUpdated) {
          DMSG ((EFI_D_ERROR, "\n SetupBrowser, ProcessCallBackFunction(Open), Hii Package is Updated.\n"));
          Selection->Action = UI_ACTION_REFRESH_FORMSET;
          mHiiPackageListUpdated = FALSE;
          break;
        }
    ...
    //
    DMSG ((EFI_D_ERROR, "SetupBrowser, LoadFormSetConfig, \n"));
    Status = LoadFormSetConfig (Selection, Selection->FormSet);
    DMSG ((EFI_D_ERROR, "SetupBrowser, LoadFormSetConfig :%r.\n", Status));
    if (EFI_ERROR (Status)) {
      goto Done;
    }
    ...
    if (!mFinishRetrieveCall) {
      ...
      if (ConfigAccess != NULL) {
        ...
        DMSG ((EFI_D_ERROR, "SetupBrowser, ProcessCallBackFunction(Retrieve) :%r.\n", Status));		
        if (EFI_ERROR (Status)) {
          goto Done;
        }
        ...
        //
        if (mHiiPackageListUpdated) {
          DMSG ((EFI_D_ERROR, "\n SetupBrowser, ProcessCallBackFunction(Retrieve), Hii Package is Updated.\n"));
          Selection->Action = UI_ACTION_REFRESH_FORMSET;
          mHiiPackageListUpdated = FALSE;
          break;
        }
    ...
    // Display form
    //
    DMSG ((EFI_D_ERROR, "SetupBrowser, DisplayForm, \n"));
    gBeInteractiveStage = TRUE;
    Status = DisplayForm ();
    DMSG ((EFI_D_ERROR, "SetupBrowser, DisplayForm :%r.\n", Status));
    if (EFI_ERROR (Status)) {
      goto Done;
    }	

    if (gItemMoveAction == BROWSER_ACTION_MOVE_DOWN ) {
      Statement = Selection->Statement;
      DMSG ((EFI_D_ERROR, "SetupBrowser, ProcessCallBackFunction(Down), \n"));		
      ProcessCallBackFunction (Selection, Selection->FormSet, Selection->Form, Statement , BROWSER_ACTION_MOVE_DOWN, FALSE);
      Selection->Action = UI_ACTION_REFRESH_FORMSET;
      gItemMoveAction = 0;
      break;
    }
    if (gItemMoveAction == BROWSER_ACTION_MOVE_UP) {
      Statement = Selection->Statement;
      DMSG ((EFI_D_ERROR, "SetupBrowser, ProcessCallBackFunction(Up), \n"));		
      ProcessCallBackFunction (Selection, Selection->FormSet, Selection->Form, Statement , BROWSER_ACTION_MOVE_UP, FALSE);
      Selection->Action = UI_ACTION_REFRESH_FORMSET;
      gItemMoveAction = 0;
      break;
    }
    ...
    if (Statement != NULL) {
      if ((ConfigAccess != NULL) && 
        ...
        DMSG ((EFI_D_ERROR, "SetupBrowser, ProcessCallBackFunction(Changing) :%r.\n", Status));				
        ...
        if (!EFI_ERROR (Status) && 
            ...
            ((Statement->Operand  == EFI_IFR_CHECKBOX_OP) || (Statement->Operand  == EFI_IFR_ONE_OF_OP) || (Statement->Storage == NULL) || (Statement->Storage != NULL && Statement->ValueChanged))) {
          //
          // Only question value has been changed, browser will trig CHANGED callback.
          //
          DMSG ((EFI_D_ERROR, "SetupBrowser, ProcessCallBackFunction(Changed), \n"));				
        ...
        // Do the question validation.
        //
        DMSG ((EFI_D_ERROR, "SetupBrowser, SetQuestionValue(), \n"));		
        Status = ValueChangedValidation (gCurrentSelection->FormSet, gCurrentSelection->Form, Statement);
      ...
      if ((Status == EFI_SUCCESS) && 
        ...
        if ((Statement->QuestionFlags & EFI_IFR_FLAG_RESET_REQUIRED) != 0) {
          DMSG_HII(Statement->Prompt, Selection->Handle, L"Set Reset Flag");
          gResetRequired = TRUE;
        }
    ...
    if ((ConfigAccess != NULL) && 
      ...
      DMSG ((EFI_D_ERROR, "SetupBrowser, ProcessCallBackFunction(Close) :%r.\n", Status));	  
      if (EFI_ERROR (Status)) {
        goto Done;
      }
    }

    DMSG ((EFI_D_ERROR, "SetupBrowser, End, Selection->Action :0x%x.\n", Selection->Action));	  
