  ...
  )
{
  ...
  if(PlatSataHostIndex == 0xFFFF || PlatSataHostIndex == 0x8000){
    ...
    UnicodeSPrint(NewString, 32*sizeof(CHAR16), L"SATA%d-%d: %s", PlatSataHostIndex+1, AtaIndex+1, ModelName);
  }
...
#define MY_LANGUAGE_RFC_3066_ENGLISH ((CHAR8*)"en")    // UEFI 2.1
#define MY_LANGUAGE_ISO_639_2_ENGLISH ((CHAR8*)"eng")  // UEFI/EFI < 2.1

STATIC
CHAR16 *
MyOpalDriverGetDeviceNameByProtocol (
  EFI_HANDLE             Handle,
...
{
  EFI_HANDLE*                   ProtocolHandlesBuffer = NULL;
  ...
  if (AllHandlesBuffer == NULL || NumAllHandles == 0) {
    goto ProcExit;
  }
  ...
  if (EFI_ERROR(Status) || NumProtocolHandles == 0) {
    goto ProcExit;
  }
...
// get ptCN or ptCN2
    Status = gBS->OpenProtocol(
                 ProtocolHandlesBuffer[Index1],     // cn or cn2
                 Protocol,
                 (VOID**)&Cnp1_2,
                 gImageHandle,
                 NULL,
...
ProcExit:
  if(ProtocolHandlesBuffer != NULL){FreePool(ProtocolHandlesBuffer);}
  return DevName;
}
...
STATIC
CHAR16 * 
MyOpalDriverGetDriverDeviceName (
    EFI_HANDLE  Handle
  )
{
  EFI_HANDLE    *AllHandlesBuffer = NULL;
  UINTN         NumAllHandles;
  EFI_STATUS    Status;
  CHAR16        *DevName = NULL;
  CHAR16        *Str = NULL;
  ...
  DevName = MyOpalDriverGetDeviceNameByProtocol(Handle, AllHandlesBuffer, NumAllHandles, FALSE);
  if (DevName == NULL) {
    DevName = MyOpalDriverGetDeviceNameByProtocol(Handle, AllHandlesBuffer, NumAllHandles, TRUE);
  }
...
ProcExit:
  if(AllHandlesBuffer != NULL){FreePool(AllHandlesBuffer);}
  return Str;
}
...
STATIC CHAR16 *GetDescFromPci(EFI_HANDLE Handle, EFI_DEVICE_PATH_PROTOCOL *ptDevPath)
{
  ...
  if(ClassCode[2] == 8 && 
    ...
    DevName = MyOpalDriverGetDriverDeviceName(Handle);
    if(DevName == NULL){
      Name = L"NVME";
      Name = AllocateCopyPool(StrSize(Name), Name);
    } else {
...
// Uefi
  ...
  for (Index = 0; Index < NvBootOptionCount; Index++) {
    ...
    if(NvBootOptions[Index].OptionalDataSize == OptDataSize &&
      CompareMem(NvBootOptions[Index].OptionalData, OptData, OptDataSize)==0){
