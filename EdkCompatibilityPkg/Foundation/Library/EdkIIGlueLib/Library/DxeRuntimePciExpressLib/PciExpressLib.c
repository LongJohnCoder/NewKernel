  ...
  )
{
  ...
  //
  NewTable = AllocateRuntimeZeroPool ((mDxeRuntimePciExpressLibNumberOfRuntimeRanges + 1) * sizeof (PCI_EXPRESS_RUNTIME_REGISTRATION_TABLE));
  if (NewTable != NULL && mDxeRuntimePciExpressLibRegistrationTable != NULL) {
    CopyMem (NewTable, mDxeRuntimePciExpressLibRegistrationTable, mDxeRuntimePciExpressLibNumberOfRuntimeRanges * sizeof (PCI_EXPRESS_RUNTIME_REGISTRATION_TABLE));
    FreePool (mDxeRuntimePciExpressLibRegistrationTable);
  }
  if (NewTable == NULL) {
