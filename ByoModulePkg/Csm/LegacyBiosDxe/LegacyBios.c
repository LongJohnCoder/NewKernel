...
//LegacyBiosInitBda (Private);
  ...
  if (Regs.X.AX != 0) {
    DEBUG((EFI_D_INFO, "%a() L%d AX:%X\n", __FUNCTION__, __LINE__, Regs.X.AX));
  ...
  )
{
  ...
  //
  // Allocate space for thunker and Init Thunker under 64kb
  //
  Status = AllocateLegacyMemory (
             AllocateMaxAddress,
             0x10000,
  ...
  EfiToLegacy16InitTable->LowPmmMemorySizeInBytes = PcdGet32(PcdLowPmmMemorySize);

  MemorySize = PcdGet32 (PcdHighPmmMemorySize);
  ASSERT ((MemorySize & 0xFFF) == 0);

  HighPmmMemory = PcdGet32(PcdHighPmmMemoryAddress);
  if(HighPmmMemory == 0){
    //
    ...
    EfiToLegacy16InitTable->HiPmmMemory = HighPmmMemory;
    EfiToLegacy16InitTable->HiPmmMemorySizeInBytes = MemorySize;
  }
  
  DEBUG((EFI_D_INFO, "HiPmm(%X,%X)\n", EfiToLegacy16InitTable->HiPmmMemory, EfiToLegacy16InitTable->HiPmmMemorySizeInBytes));
  ...
  if (Regs.X.AX != 0) {
    ...
    PcdSet32 (PcdEndOpromShadowAddress, (Regs.X.DS * 16 + Regs.X.BX - 0x2000)); // Reserve 8k for some tables use
