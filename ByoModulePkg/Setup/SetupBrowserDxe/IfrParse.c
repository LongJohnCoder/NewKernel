    ...
    if ((BrowserStorage->Type == StorageType) && CompareGuid (&BrowserStorage->Guid, StorageGuid)) {
      if (StorageType == EFI_HII_VARSTORE_NAME_VALUE) {
        if (BrowserStorage->HiiHandle == HiiHandle) {
          DMSG ((EFI_D_ERROR, "FindStorageInList, Get Browser Storage :%d-%s.\n", StorageType, StorageName));
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "\n CreateStorage, Create Formset Storage.\n"));
  ...
  if (BrowserStorage == NULL) {
    ...
    DMSG ((EFI_D_ERROR, "CreateStorage, Create New Browser Storage :%d-%s.\n", StorageType, UnicodeString));
  ...
  )
{
  ...
  FreePool (FormSet);
  DMSG ((EFI_D_ERROR, "DestroyFormSet, End. \n"));
}
  ...
  )
{
  ...
  while (OpCodeOffset < FormSet->IfrBinaryLength) {
    ...
    // Parse the Opcode
    // 
    //DMSG ((EFI_D_ERROR, "ParseOpCodes, Operand :0x%x. \n", Operand));
    switch (Operand) {
  ...
  DMSG ((EFI_D_ERROR, "ParseOpCodes, End. \n"));
  return EFI_SUCCESS;
}
