  ...
  )
{
  ...
  UINT8               Data8;
  BOOLEAN             HasEfiImage;
  BOOLEAN             HasLegacyImage;
  UINT8               OptionRomPost;
  BOOLEAN             NvmeAddOnOpRom;
  ...
  if (PcdGet8(PcdLegacyBiosSupport)) {
    HasLegacyImage = ContainLegacyImage(PciIoDevice->PciIo.RomImage, PciIoDevice->PciIo.RomSize);;
  } else {
    HasLegacyImage = FALSE;
  }

  switch(PciIoDevice->Pci.Hdr.ClassCode[2]){
    case PCI_CLASS_DISPLAY:
      OptionRomPost = PcdGet8(PcdVideoOpRomLaunchPolicy);
      break;

    case PCI_CLASS_NETWORK:
      OptionRomPost = PcdGet8(PcdPxeOpRomLaunchPolicy);
      break;

    case PCI_CLASS_MASS_STORAGE:
      OptionRomPost = PcdGet8(PcdStorageOpRomLaunchPolicy);
      break;  

    default:
      OptionRomPost = PcdGet8(PcdOtherOpRomLaunchPolicy);
      break;
  }
...
// uefi   : 1
// no     : 2
  if(OptionRomPost == 0 && HasLegacyImage){    // legacy first && has legacy
    HasEfiImage = 0;
  } else if(OptionRomPost == 2){               // no launch
    HasEfiImage = 0;
  } else {
    HasEfiImage = ContainEfiImage(PciIoDevice->PciIo.RomImage, PciIoDevice->PciIo.RomSize);
  }

  NvmeAddOnOpRom = (PcdGet32(PcdSystemMiscConfig) & SYS_MISC_NVME_ADDON_OPROM)?1:0;
  if(!NvmeAddOnOpRom){
    if(PciIoDevice->Pci.Hdr.ClassCode[2] == 1 &&  // BIOS first so ignore NVME Addon option rom
       PciIoDevice->Pci.Hdr.ClassCode[1] == 8 && 
       PciIoDevice->Pci.Hdr.ClassCode[0] == 2){
      HasEfiImage = 0;
    }
  }
  
  DEBUG((EFI_D_INFO, "PCI(%X,%X,%X) %04X%04X L:%d E:%d P:%d C:%X ROM(%X,%X)\n", \
    PciIoDevice->BusNumber, PciIoDevice->DeviceNumber, PciIoDevice->FunctionNumber, \
    PciIoDevice->Pci.Hdr.DeviceId, PciIoDevice->Pci.Hdr.VendorId, \
    HasLegacyImage, HasEfiImage, OptionRomPost, PciIoDevice->Pci.Hdr.ClassCode[2], \
    PciIoDevice->PciIo.RomImage, PciIoDevice->PciIo.RomSize));
