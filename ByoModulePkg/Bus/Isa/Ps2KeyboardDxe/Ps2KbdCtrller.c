  ...
  )
{
  ...
  if(ConsoleIn->LastLedState == *(UINT8*)BdaLed){
    return EFI_SUCCESS;
  }
  ...
  ConsoleIn->LastLedState = *(UINT8*)BdaLed;
  ...
  )
{
  ...
  if(ExtendedVerification){
    ...
    DEBUG((EFI_D_INFO, "CheckKeyboardConnect:%d\n", Rc));
  } else {
    Rc = FALSE;
  }	
...
//
// ConSplitterTextInExAddDevice() will sync LED status
//
/*  
  if (Ps2Policy != NULL) {
