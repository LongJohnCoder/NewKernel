...
VOID CheckFixedApVectorSupport(CPU_MP_DATA *CpuMpData)
{
  UINT32  S3ApVectorAddr;
  UINT16  S3ApVectorSize;


  if(CpuMpData->FixedApVectorChecked){
    return;
  }

  S3ApVectorAddr = PcdGet32(PcdCpuS3ApVectorAddress);
  S3ApVectorSize = PcdGet16(PcdCpuS3ApVectorMaxSize);

  if(S3ApVectorSize != 0){
    ASSERT(S3ApVectorAddr != 0 && S3ApVectorAddr != 0xFFFFFFFF);
    CpuMpData->FixedApVector     = S3ApVectorAddr;
    CpuMpData->FixedApVectorSize = S3ApVectorSize;
  }

  CpuMpData->FixedApVectorChecked = TRUE;
}
  ...
  )
{
  ...
  DEBUG((EFI_D_INFO, "%a(%d,%d)\n", __FUNCTION__, Broadcast, ProcessorNumber));

  CpuMpData->FinishedCount = 0;
  ResetVectorRequired = FALSE;

  CheckFixedApVectorSupport(CpuMpData);

  DEBUG((EFI_D_INFO, "ApLoopMode:%d, InitFlag:%d\n", CpuMpData->ApLoopMode, CpuMpData->InitFlag));  
  ...
  )
{
  ...
  if (OldCpuMpData == NULL) {
    if (MaxLogicalProcessorNumber > 1) {
      PrepareMicroCode(CpuMpData);
      CollectProcessorCount (CpuMpData);
      for(Index=0;Index<CpuMpData->CpuCount;Index++){
        if(Index == CpuMpData->BspNumber){
          continue;
        }
        WakeUpAP(CpuMpData, FALSE, Index, LoadApMicroCode, CpuMpData);
        while (CpuMpData->FinishedCount < 1) {
          CpuPause ();
        }
        SetApState (&CpuMpData->CpuData[Index], CpuStateIdle);
      ...
      #ifdef ZX_SECRET_CODE
	  {
		 ...
		 CPU_INFO_IN_HOB      *CpuInfoInHob;
		 CpuInfoInHob = (CPU_INFO_IN_HOB *)(UINTN)CpuMpData->CpuInfoInHob;
		 CoreEnabled = (UINT8)PcdGet8(PcdCpuCoreEnabled);
		 if(CoreEnabled &&(CpuMpData->CpuCount > CoreEnabled))
		 {
		
		   DEBUG((EFI_D_ERROR,"Need kill %x cores.\n",CpuMpData->CpuCount - CoreEnabled));
		   FinishedCount = &CpuMpData->FinishedCount;
		   for(i=CpuMpData->CpuCount - 1; i>= CoreEnabled; i--) {
		     DEBUG((EFI_D_ERROR,"i=%x, Id=%x\n",i,CpuInfoInHob[i].ApicId));
		     WakeUpAP (CpuMpData, FALSE, i, KillAp, CpuMpData);
