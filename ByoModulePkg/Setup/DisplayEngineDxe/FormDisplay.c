...
BOOLEAN    gDateHighLight = FALSE;
BOOLEAN    gTimeHighLight = FALSE;
...
MOUSE_ACTION_INFO    gMouseAction;
  ...
  )
{
  ...
  if (String == NULL) {
    String = AllocateCopyPool (StrSize (L"!"), L"!");
    ASSERT (String != NULL);
  }
  ...
  )
{
  ...
  gConfirmEscExitMsg    = GetToken (STRING_TOKEN (CONFIRM_ESC_EXIT_MESSAGE), gHiiHandle);
}
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "\n GetHotKeyFromRegisterList, FormDisplay,\n"));
  Link = GetFirstNode (&gFormData->HotKeyListHead);
  while (!IsNull (&gFormData->HotKeyListHead, Link)) {
    HotKey = BROWSER_HOT_KEY_FROM_LINK (Link);
    
    if (HotKey->KeyData->ScanCode != 0 && HotKey->KeyData->ScanCode == KeyData->ScanCode) {
     DMSG ((EFI_D_ERROR, "HotKey :0x%x. \n", KeyData->ScanCode));
      return HotKey;
    }
  ...
  )
{
  ...
  if (gSetupMouse && PcdGetBool(PcdTextSetupMouseEnable)) {
    WaitList[1] = gSetupMouse->WaitForMouse;
    EventNum    = 2;
  } else {
    EventNum    = 1;
  }
  ...
  if (gSetupMouse && PcdGetBool(PcdTextSetupMouseEnable)) {
    ...
    case 2:
      if (gFormData->FormRefreshEvent != NULL) {
        ...
        ASSERT (Timeout != 0 && EventNum == 3);
        EventType = UIEventTimeOut;
      }
      break;
  
    default:
      ASSERT (Index == 3 && EventNum == 4);
      EventType = UIEventTimeOut;
      break;
    }
  ...
  if (Link->BackLink == &gMenuOption) {
    ...
    if (PreviousMenuOption != NULL && Rows < PreviousMenuOption->Skip) {
      *SkipValue = PreviousMenuOption->Skip - Rows;
      if (TotalRows < (PreviousMenuOption->Skip + Rows)) {
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "\n FxConfirmPopup, Action :0x%x. \n", Action));
  //
  // Retrieve global string again to adapt to change of language.
  //
  FreeDisplayStrings ();
  InitializeDisplayStrings();  
  ...
  if ((Action & BROWSER_ACTION_DEFAULT) == BROWSER_ACTION_DEFAULT) {
    ...
    CfmStrLen += StrLen (gStrUiLoadDefault);
  }
  ...
  if ((Action & BROWSER_ACTION_DEFAULT) == BROWSER_ACTION_DEFAULT) {
    if (CfmStr[0] != 0) {
      ...
      StrCpyS (CfmStr, MaxLen, gStrUiLoadDefault);
    }
  }
  ...
  RetVal = FALSE;
  if (SELECTION_YES == UiConfirmDialog(DIALOG_YESNO, gConfirmOpt, NULL, TEXT_ALIGIN_CENTER, gEmptyString, CfmStr, NULL)) {
    RetVal = TRUE;
  }
  FreePool (CfmStr);
...
/**
  ...
  @param  ScrollBar                Whether need to show ScrollBar.
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "DisplayOneMenu, MenuOption :%s. \n", MenuOption->Description));  
  //
  // Set default color.
  //
  SetDisplayAttribute (MenuOption, FALSE);
      ...
      if (((Temp2 == 0)) && (Row <= BottomRow)) {
        if (Statement->OpCode->OpCode == EFI_IFR_DATE_OP || Statement->OpCode->OpCode == EFI_IFR_TIME_OP) {
          ...
          if (MenuOption->Skip != 0) {
            ...
            //
            DisplayMenuString (MenuOption,MenuOption->OptCol, Row, OutputString, Width + 1 - 7 - ScrollBar -1, Highlight);
          } else {
            //
          ...
          DisplayMenuString (MenuOption, MenuOption->OptCol, Row, OutputString, Width, Highlight);
        }
        OptionLineNum++;
      }
  ...
  if (gDateHighLight && (EFI_IFR_DATE_OP == MenuOption->ThisTag->OpCode->OpCode)) {
    SetDisplayAttribute (MenuOption, TRUE);
  }else if (gTimeHighLight && (EFI_IFR_TIME_OP == MenuOption->ThisTag->OpCode->OpCode)) {
    SetDisplayAttribute (MenuOption, TRUE);
  }
  
  if (MenuOption->Description == NULL || MenuOption->Description[0] == '\0') {
    //
    //Clear this line to avoid residual characters.
    //
    ClearWithBlt(BeginCol, PromptWidth + AdjustValue + SkipWidth + 1, Row, Row, EFI_BACKGROUND_LIGHTGRAY);
    PromptLineNum++;
  } else {
  ...
  if (HasOptionString(MenuOption) && (OptionLineNum != PromptLineNum)) {
    ...
    ClearWithBlt (Col, Col + Width, Row, MaxRow, EFI_BACKGROUND_LIGHTGRAY);
  }
...
VOID
PrintDebugInfoOfControlFlag	 (
  UI_CONTROL_FLAG    Flag
  )
{
  #ifdef SETUP_DEBUG
  switch (Flag) {
    case CfInitialization:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfInitialization, \n"));
      break;
    case CfCheckSelection:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfCheckSelection, \n"));
      break;
    case CfRepaint:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfRepaint, \n"));
      break;
    case CfRefreshHighLight:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfRefreshHighLight, \n"));
      break;
    case CfUpdateHelpString:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUpdateHelpString, \n"));
      break;
    case CfPrepareToReadKey:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfPrepareToReadKey, \n"));
      break;
    case CfReadKey:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfReadKey, \n"));
      break;
    case CfScreenOperation:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfScreenOperation, \n"));
      break;
    case CfUiSelect:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiSelect, \n"));
      break;
    case CfUiReset:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiReset, \n"));
      break;
    case CfUiLeft:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiLeft, \n"));
      break;
    case CfUiRight:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiRight, \n"));
      break;
    case CfUiUp:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiUp, \n"));
      break;
    case CfUiPageUp:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiPageUp, \n"));
      break;
    case CfUiPageDown:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiPageDown, \n"));
      break;
    case CfUiDown:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiDown, \n"));
      break;
    case CfUiNoOperation:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiNoOperation, \n"));
      break;
    case CfExit:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfExit, \n"));
      break;
    case CfUiHotKey:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiHotKey, \n"));
      break;
    case CfUiTab:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiTab, \n"));
      break;
    case CfUiHelp:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfUiHelp, \n"));
      break;
    case CfItemRefresh:
      DMSG ((EFI_D_ERROR, "UiDisplayMenu, CfItemRefresh, \n"));
      break;
    default:
      break;
  }
...
/**
Check System Password Enter Status, return TRUE when power on password entered.
  ...
  )
{
  ...
  if (!EFI_ERROR(Status) && SystemPassword->GetEnteredType) {
    if (LOGIN_USER_POP == SystemPassword->GetEnteredType()) {
      return TRUE;
    }
  }
  ...
  )
{
  ...
  BOOLEAN    bShowScrollBar = FALSE;
  UINTN    TotalSkipLines;
  UINTN    TopSkipLines;
  UI_MENU_OPTION    *TmpMenuOption;  
  
  LIST_ENTRY    *TmpMenuPos;
  EFI_TPL    OriginalSetupTpl = TPL_APPLICATION;

  MOUSE_CURSOR_RANGE    ScrollBarRange_Up, ScrollBarRange_Down;
  ...
  if (gClassOfVfr == FORMSET_CLASS_PLATFORM_SETUP) {
    ...
    BottomRow = gStatementDimensions.BottomRow - SCROLL_ARROW_HEIGHT;
    HelpTopRow = gStatementDimensions.TopRow + SCROLL_ARROW_HEIGHT;
  } else {
  ...
  while (TRUE) {
    ...
    case CfRepaint:
      ...
      DMSG_HII (FormData->FormTitle, FormData->HiiHandle, L"FormTitle");
      if (Repaint) {
        //
        // Display menu
        //
        ...
        // Repaint middle line.
        //
        RepaintMiddleLine  (FormData, (gStatementDimensions.RightColumn - gHelpBlockWidth - 1));
        ...
        } else if (gClassOfVfr == FORMSET_CLASS_FRONT_PAGE) {
        ...
        if (UpArrow && (gClassOfVfr == FORMSET_CLASS_FRONT_PAGE)) {
        ...
        for (Link = gMenuOption.ForwardLink; Link != &gMenuOption; Link = Link->ForwardLink) {
          ...
          UpdateOptionSkipLines (TmpMenuOption);
        ...
        // 2.Paint the menu.
        //
        gDateHighLight = FALSE;
        gTimeHighLight = FALSE;
        ...
        for (Link = TopOfScreen; Link != &gMenuOption; Link = Link->ForwardLink) {
          ...
          if ((FormData->Attribute & HII_DISPLAY_MODAL) != 0) {
            ...
            Statement = MenuOption->ThisTag;
            if (gDateHighLight && IsSelectable(MenuOption)) {
              if (EFI_IFR_DATE_OP != Statement->OpCode->OpCode) {
                gDateHighLight = FALSE;                
              }
            }
            if (gTimeHighLight && IsSelectable(MenuOption)) {
              if (EFI_IFR_TIME_OP != Statement->OpCode->OpCode) {
                gTimeHighLight = FALSE;
              }
            }
            ...
            if ((Link == NewPos) && IsSelectable(MenuOption)) {
              if (EFI_IFR_DATE_OP == Statement->OpCode->OpCode) {
                gDateHighLight = TRUE;
                gTimeHighLight = FALSE;
              }
        ...
        } else if (gClassOfVfr == FORMSET_CLASS_PLATFORM_SETUP){
          ClearWithBlt (gStatementDimensions.LeftColumn, gStatementDimensions.RightColumn - gHelpBlockWidth - gStatementDimensions.LeftColumn -bShowScrollBar, Row, BottomRow, EFI_BACKGROUND_LIGHTGRAY);
        } else {
          ClearWithBlt (gStatementDimensions.LeftColumn, gStatementDimensions.RightColumn - gHelpBlockWidth - gStatementDimensions.LeftColumn +1, Row, BottomRow, EFI_BACKGROUND_LIGHTGRAY);
        }        
        ...
        if (DownArrow && (gClassOfVfr == FORMSET_CLASS_FRONT_PAGE)) {
          gST->ConOut->SetAttribute (gST->ConOut, GetArrowColor ());
          PrintCharAt (
            gStatementDimensions.LeftColumn + gPromptBlockWidth + gOptionBlockWidth + 1,
        ...
        // 5.Show Scroll bar.
        //
        if (bShowScrollBar && (gClassOfVfr == FORMSET_CLASS_PLATFORM_SETUP)) {
            DrawScrollBar (
                  (gStatementDimensions.RightColumn - gHelpBlockWidth - 1) - 1,
                  TopRow,
                  BottomRow - TopRow + 1,
                  TopSkipLines,
                  TotalSkipLines
                  );			
            SetRange(&ScrollBarRange_Up, (gStatementDimensions.RightColumn - gHelpBlockWidth - 1) - 1, 1, TopRow, 2);
            SetRange(&ScrollBarRange_Down, (gStatementDimensions.RightColumn - gHelpBlockWidth - 1) - 1, 1, BottomRow -1, 2);
        }else if (gClassOfVfr == FORMSET_CLASS_PLATFORM_SETUP) {
          gST->ConOut->SetAttribute (gST->ConOut, EFI_BLUE | EFI_BACKGROUND_LIGHTGRAY);
          for(Index = TopRow; Index <= BottomRow; Index++) {
            PrintCharAt((gStatementDimensions.RightColumn - gHelpBlockWidth - 1) - 1, Index, L' ');
          }
        }
    ...
    case CfItemRefresh:
      ...
      if (gFormData->FormRefreshEvent) {
        ...
        for (Link = TopOfScreen; Link != &gMenuOption; Link = Link->ForwardLink) {
          ...
          //
          // Update the row info which will be used by next menu.
          //
          if (Link == TopOfScreen) {
            Row += MenuOption->Skip - SkipValue;
          } else {
      ...
      gBS->RestoreTPL(OriginalSetupTpl);
      break;
    ...
    case CfUpdateHelpString:
      ...
      if (Repaint || NewLine) {
        ...
        RowCount      = BottomRow - HelpTopRow + 1;
        HelpPageIndex = 0;
        //
      ...
      if (!MultiHelpPage) {
        ...
        if (HelpPageIndex == 0) {
          ...
          for (Index = 0; (Index < RowCount - HelpBottomLine - HelpHeaderLine) && 
            ...
            PrintStringAtWithWidth (
              gStatementDimensions.RightColumn - gHelpBlockWidth,
              Index + HelpTopRow + HelpHeaderLine,
    ...
    case CfReadKey:
      ...
      if (CHAR_TAB == gReturnedDirection) {
        gReturnedDirection = 0;
      ...
      while (TRUE) {
        ...
        if (!EFI_ERROR (Status)) {
          ...
          while (gST->ConIn) {
            ...
            DMSG ((EFI_D_ERROR, "Clear Key Buffer :0x%x-%c. \n", KeyTmp.ScanCode, KeyTmp.UnicodeChar));
          }		  
          EventType = UIEventKey;
          break;
        }
        ...
        if (EventType == UIEventKey) {
          Status = gST->ConIn->ReadKeyStroke (gST->ConIn, &Key);
          if (!EFI_ERROR (Status)) {
            DMSG ((EFI_D_ERROR, "UIEventKey, Get Key :%d-%c. \n", Key.ScanCode, Key.UnicodeChar));
            while (gST->ConIn) {
              //
              // clean key buffer.
              //
              ...
              DMSG ((EFI_D_ERROR, "UIEventKey, Clear Key Buffer :0x%x-%c. \n", KeyTmp.ScanCode, KeyTmp.UnicodeChar));
            }
          }
        }
        if (EventType == UIEventMouse) {
          gSetupMouse->GetData(&gMouseAction);				
        }
        break;
      }
      ...
      if (EventType == UIEventDriver) {
        ...
        DMSG ((EFI_D_ERROR, "Refresh Dynamic item, FormRefreshEvent :0x%x.\n", gFormData->FormRefreshEvent));
        ControlFlag = CfItemRefresh;
        OriginalSetupTpl = gBS->RaiseTPL(TPL_CALLBACK);
        DMSG ((EFI_D_ERROR, "UIEventDriver, OriginalSetupTpl :%d.\n", OriginalSetupTpl));
        FormData->RefreshItemFlag = 0;
        break;
      }
      ...
      if ( EventType == UIEventMouse) {
        ...
        if (gMouseAction.Action == MOUSE_RIGHT_CLICK ) {
          ControlFlag = CfUiReset;
          break;
        }
        switch (GetSetupEreaType(&gMouseAction, FormData->ByoLayoutStyle)) {
          case SETUP_AREA_TITLE:
            if (gMouseAction.Action == MOUSE_LEFT_CLICK && MouseGotoFormset (&gMouseAction)) {
              ControlFlag = CfExit;
            }
            break;
		  
          case SETUP_AREA_MENU:
            //
            //Response Scroll Bar Action.
            //
            if (bShowScrollBar) {          
              if (IsMouseInRange(&ScrollBarRange_Up, &gMouseAction)) {
                Key.UnicodeChar = CHAR_NULL;
                Key.ScanCode = SCAN_UP;
                break;
              }

              if (IsMouseInRange(&ScrollBarRange_Down, &gMouseAction)) {
                Key.UnicodeChar = CHAR_NULL;
            ...
            TmpMenuPos = NULL;
            TmpMenuPos = MouseGetMenuOption(&gMouseAction, TopOfScreen);			
            if (TmpMenuPos) {

              if (gMouseAction.Action == MOUSE_LEFT_CLICK) {
                if (NewPos != TmpMenuPos) {
                  ...
                  gMouseAction.Action = MOUSE_LEFT_DOUBLE_CLICK;
                }
              }

              if (gMouseAction.Action == MOUSE_LEFT_DOUBLE_CLICK) {
                if (NewPos != TmpMenuPos) {
                  MenuOption = MENU_OPTION_FROM_LINK (NewPos);
                  NewPos = TmpMenuPos;
                  Repaint = TRUE;
                  ...
                  NewPos = TmpMenuPos;
                  gUserInput->SelectedStatement = MenuOption->ThisTag;
                  Repaint = TRUE;
                  NewLine = TRUE;		
                  ControlFlag = CfUiSelect;
                }
          ...
          case SETUP_AREA_HOTKEY:
            MouseGetHotkey (&gMouseAction, &Key);
            break;
      ...
      case CHAR_CARRIAGE_RETURN:
        ...
        DMSG ((EFI_D_ERROR, "EventType, CHAR_CARRIAGE_RETURN, \n"));
        gUserInput->Action = BROWSER_SUCCESS;
      ...
      case '-':
        ...
        if ((Statement->OpCode->OpCode == EFI_IFR_DATE_OP)
          ...
          if (EFI_ERROR (Status)) {
            ...
            ControlFlag = CfExit;
            FormData->RefreshItemFlag = 2;
          }
        }

        if (Statement->OpCode->OpCode == EFI_IFR_ACTION_OP) {
          if (Key.UnicodeChar == '+') {
            gUserInput->Action = BROWSER_ACTION_MOVE_UP;
            ControlFlag = CfExit;
          } else {
            gUserInput->Action = BROWSER_ACTION_MOVE_DOWN;
            ControlFlag = CfExit;
          }
        }
        break;
      ...
      case CHAR_NULL:
        ...
        if (((FormData->Attribute & HII_DISPLAY_MODAL) != 0) && (Key.ScanCode == SCAN_ESC || Index == mScanCodeNumber)) {
          ...
          if (HotKey != NULL) {
            DMSG ((EFI_D_ERROR, "CfReadKey, HotKey :0x%x.\n", HotKey->KeyData->ScanCode));
            ScreenOperation = UiHotKey;
            if(BePopEntered()) {
              DMSG ((EFI_D_ERROR, "CfReadKey, Pop Entered.\n"));
              if (HotKey->KeyData->ScanCode != SCAN_F1) {
                HotKey = NULL;
                ScreenOperation = UiNoOperation;  
              }
            }
    ...
    case CfUiSelect:
      ...
      ASSERT(MenuOption != NULL);
      Statement = MenuOption->ThisTag; 
      DMSG ((EFI_D_ERROR, "CfUiSelect, Statement :%s. \n", MenuOption->Description));  
      if (Statement->OpCode->OpCode == EFI_IFR_TEXT_OP) {
        break;
      }
	  
      DMSG ((EFI_D_ERROR, "CfUiSelect, OpCode :0x%x. \n", Statement->OpCode->OpCode));
      switch (Statement->OpCode->OpCode) {
      case EFI_IFR_REF_OP:
      ...
      default:
        ...
        //
        FormData->RefreshItemFlag = 1;
        gUserInput->Action = 0;
        RefreshKeyHelp (gFormData, Statement, TRUE);
        Status = ProcessOptions (MenuOption, TRUE, &OptionString, TRUE);
        DMSG ((EFI_D_ERROR, "CfUiSelect, OptionString :%s. \n", OptionString));
    ...
    case CfUiReset:
      ...
      if ((FirstFormId == FormData->FormId && NULL != FormData->ByoCurrentFormSetLink) && IsByoFormset(FormData->ByoFormSetList, FormData->HiiHandle)) {
        Status = gBS->LocateProtocol (
        ...
        if (!EFI_ERROR(Status)) {
          if (BrowserExtension->IsBrowserDataModified()) {
            SaveFlag = ConfirmDataChange(NULL);  
            if (SaveFlag & BROWSER_ACTION_SUBMIT) {
              gUserInput->Action = BROWSER_ACTION_SUBMIT | BROWSER_ACTION_EXIT;
              ControlFlag = CfExit;
              break;
            }else if (SaveFlag & BROWSER_ACTION_NONE) {
              ControlFlag = CfRepaint;
              Repaint = TRUE;
              gUserInput->Action = BROWSER_ACTION_NONE;
              break;
            }else if (SaveFlag & BROWSER_ACTION_DISCARD) {
              ControlFlag = CfRepaint;
              Repaint = TRUE;
              gUserInput->Action = BROWSER_ACTION_NONE;
              break;
            ...
            SaveFlag = ConfirmDataChange(gConfirmEscExitMsg);
            if (SaveFlag & BROWSER_ACTION_SUBMIT) {
    ...
    case CfUiUp:
      ...
      if (Difference < 0) {
        ...
        if (TopOfScreen != gMenuOption.ForwardLink || SkipValue != 0) {
          ...
          //
          // Keep on top when we have arrived at top page.
          //
          TopOfScreen = gMenuOption.ForwardLink;
          NewPos      = SavedListEntry;
          SkipValue = 0;
  ...
  )
{
  ...
  case BROWSER_RECONNECT_SAVE_CHANGES:
    ...
    do {
      UiConfirmDialog(DIALOG_WAIT_KEY, ErrorInfo, &Key, TEXT_ALIGIN_CENTER, gEmptyString, PrintString, NULL);
  ...
  default:
    if (TimeOut == 0) {
      do {
        UiConfirmDialog(DIALOG_WAIT_KEY, ErrorInfo, &Key, TEXT_ALIGIN_CENTER, gEmptyString, gPressEnter, NULL);
  ...
  )
{
  ...
  //
  DMSG ((EFI_D_ERROR, "FormDisplay, BrowserStatus, \n"));
  BrowserStatusProcess();
  DMSG ((EFI_D_ERROR, "FormDisplay, BrowserStatus :0x%x.\n", gFormData->BrowserStatus));
  if (gFormData->BrowserStatus != BROWSER_SUCCESS) {
    //
  ...
  DMSG ((EFI_D_ERROR, "FormDisplay, DisplayPageFrame, \n"));
  Status = DisplayPageFrame (FormData, &gStatementDimensions);
  DMSG ((EFI_D_ERROR, "FormDisplay, DisplayPageFrame :%r.\n", Status));
  if (EFI_ERROR (Status)) {
    return Status;
  }
  ...
  //
  if (gClassOfVfr == FORMSET_CLASS_PLATFORM_SETUP) {
    gOptionBlockWidth = (CHAR16) ((gStatementDimensions.RightColumn - gStatementDimensions.LeftColumn) / 3) + 1 - 2;
  ...
  if (gClassOfVfr == FORMSET_CLASS_PLATFORM_SETUP) {
    PrintFormLayout (FormData, (gStatementDimensions.RightColumn - gHelpBlockWidth - 1));
  }
  ...
  DMSG ((EFI_D_ERROR, "FormDisplay, UiDisplayMenu, \n"));  
  Status = UiDisplayMenu(FormData);
  DMSG ((EFI_D_ERROR, "FormDisplay, UiDisplayMenu :%r. \n", Status));  
  ...
  DMSG ((EFI_D_ERROR, "FormDisplay, End. \n"));
  ...
  )
{
  ...
  bF3F4 = PcdGetBool (PcdEnableSetupHotkeyF3F4);
  DMSG ((EFI_D_ERROR, "InitializeDisplayEngine, bF3F4 :%d.\n", bF3F4));
  //
  ...
  if (!EFI_ERROR (Status)) {
    ...
    if (bF3F4) {
      ...
      NewString         = HiiGetString (gHiiHandle, STRING_TOKEN (FUNCTION_FOUR_STRING), NULL);
    } else {
      HotKey.ScanCode   = SCAN_F10;
      NewString         = HiiGetString (gHiiHandle, STRING_TOKEN (FUNCTION_TEN_STRING), NULL);
    }
    ASSERT (NewString != NULL);
    FormBrowserEx2->RegisterHotKey (&HotKey, BROWSER_ACTION_SUBMIT | BROWSER_ACTION_EXIT, 0, NewString);
    FreePool (NewString);	

    if (bF3F4) {
      HotKey.ScanCode   = SCAN_F3;
      NewString         = HiiGetString (gHiiHandle, STRING_TOKEN (FUNCTION_THREE_STRING), NULL);
    } else {
