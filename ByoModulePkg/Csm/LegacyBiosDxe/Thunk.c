...
VOID IntVectorCheck(BOOLEAN In, CONST EFI_IA32_REGISTER_SET *Regs)
{
  UINT32      *Data32;
  UINT32      Addr;	
  UINTN       Index;
...
// INT 43H: EGA/VGA Graphics Character Font Pointer
  ...
  for(Index=0;Index<0x78;Index++){
    ...
    if(Index >= 0x68 && Index <= 0x6F){continue;}
    ...
    if(Addr < 0xF0000){
      ...
      if((Index >= 0x68 && Index < 0x78) || Index == 0x13){
        if(Addr > 0xC0000){
          continue;
        }					
      }				
