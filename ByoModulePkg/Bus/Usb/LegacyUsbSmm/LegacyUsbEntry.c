...
)
{
    ...
    USB_MOUSE_COMMUNICATION_PROTOCOL   *UsbMouseCommunicationInstance;
    ...
    if (EFI_ERROR (Status)) {
      ...
      DEBUG((EFI_D_ERROR, "LegacyUsbEntryPoint.mUsbMemAddr:%lX,P:%X\n", mUsbMemAddr, Pages));
    }
    ...
    if (PcdGetBool(PcdTextSetupMouseEnable)){
      Status = gBS->AllocatePool (
                      ...
                      &UsbMouseCommunicationInstance
                      );
      if (EFI_ERROR (Status)) {
        DEBUG((EFI_D_ERROR, "BYO USB Mouse DXE Supporte Fail! \n"));
      } else {
        DEBUG((EFI_D_INFO, "BYO USB Mouse DXE Supporte %p \n", UsbMouseCommunicationInstance));
        UsbMouseCommunicationInstance->MouseDataStatus = 0x00;
        UsbMouseCommunicationInstance->MouseDataSize   = 0x04; //Hard Code for craft code
