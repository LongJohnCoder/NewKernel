  ...
  )
{
  ...
  while (!IsUnicodeFiFoEmpty (TerminalDevice) && !IsEfiKeyFiFoFull (TerminalDevice)) {
    ...
    //debug bytt.
    //DEBUG ((EFI_D_INFO, "bytt,TermType:%x, InputState:%x, unicodeChar:%x\n", TerminalDevice->TerminalType, TerminalDevice->InputState, UnicodeChar));
    switch (TerminalDevice->InputState) {
    case INPUT_STATE_DEFAULT:
      break;
      
    //add by bytt. for console redirection function key support.
    case INPUT_STATE_F5_F12:
        switch (UnicodeChar) {
          case '5':
      ...
      if (Key.ScanCode != SCAN_NULL) {
        //DEBUG ((EFI_D_INFO, "bytt,send f5-f12.\n"));
        Key.UnicodeChar = 0;
        EfiKeyFiFoInsertOneKey (TerminalDevice, &Key);
        //
        // Cancel the timer.
...
// according to http://aperiodic.net/phil/archives/Geekery/term-function-keys.html
//key	VT100
//F1	^[OP
//F2	^[OQ
//F3	^[OR
    ...
    case INPUT_STATE_ESC | INPUT_STATE_LEFTOPENBRACKET:
      ...
      if (TerminalDevice->TerminalType == TerminalTypePcAnsi    ||
        ...
        case '2':
          if (TerminalDevice->TerminalType == TerminalTypeVt100 ||
            TerminalDevice->TerminalType == TerminalTypeVtUtf8) {
            //DEBUG ((EFI_D_INFO, "bytt, continue for function f5-f12.\n"));
            TerminalDevice->InputState = INPUT_STATE_F5_F12;
            continue;
          }
          break;
        //end of add.
