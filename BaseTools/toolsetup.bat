...
set TOOLS_DEF_TEMPLATE=%EDK_TOOLS_PATH%\Conf\tools_def.template
set TOOLS_DEF_TEMPLATE_OVERRIDE=%WORKSPACE%\PlatformPkg\Override\BaseTools\Conf\tools_def.template
if exist %TOOLS_DEF_TEMPLATE_OVERRIDE% set TOOLS_DEF_TEMPLATE=%TOOLS_DEF_TEMPLATE_OVERRIDE%

if NOT exist %CONF_PATH%\tools_def.txt (
  echo copying %TOOLS_DEF_TEMPLATE% to %CONF_PATH%\tools_def.txt
  if NOT exist %TOOLS_DEF_TEMPLATE% (
    echo Error: tools_def.template is missing at %TOOLS_DEF_TEMPLATE%
  )
  copy %TOOLS_DEF_TEMPLATE% %CONF_PATH%\tools_def.txt > nul
) else (
  if defined RECONFIG echo over-write ... %TOOLS_DEF_TEMPLATE% to %CONF_PATH%\tools_def.txt
  if defined RECONFIG copy /Y %TOOLS_DEF_TEMPLATE% %CONF_PATH%\tools_def.txt > nul
)
