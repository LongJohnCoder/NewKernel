...
/*
    case GPIType:
      InsertTailList (&mPrivateData.CallbackDataBase, &Record->Link);
      CopyMem(&Record->SrcDesc, &GPI_SOURCE_DESC, sizeof(SB_SMM_SOURCE_DESC));
      Record->ClearSource = GpiSmiClearSrc;
      break;
...
//- DEBUG((EFI_D_INFO, "SwVal <- %X\n", Record->ChildContext.Sw.SwSmiInputValue));
  ...
  if (Record->ClearSource == NULL) {
    //
    // Clear the SMI associated with the source using the default function
    //
    ...
    while ((!SmiClear) && (EscapeCount > 0)) {
      ...
      while (!IsNull (&mPrivateData.CallbackDataBase, LinkInDb)) {
        ...
        if (!SourceIsActive (&RecordInDb->SrcDesc)) {
          ...
          //
          if (RecordInDb->ProtocolType == SxType || RecordInDb->ProtocolType == PowerButtonType) {
          ...
          while (!IsNull (&mPrivateData.CallbackDataBase, LinkToExhaust)) {
            ...
            if (CompareSources (&RecordToExhaust->SrcDesc, &ActiveSource)) {
              ...
              if (ContextsMatch) {
                ...
                if (RecordToExhaust->ProtocolType == SxType || RecordToExhaust->ProtocolType == PowerButtonType) {
                  SxChildWasDispatched = TRUE;
                }
              }
            }
