  ...
  )
{
  ...
  // after XHC reset, we must set bios owner ship  
  XhcSetBiosOwnership (Xhc);
  // after XHC reset, we must clear XHC_PORTSC_WRC
  PortNumber = (UINT8) (Xhc->HcSParams1.Data.MaxPorts);
	for (PortNumber = 0; PortNumber < Xhc->HcSParams1.Data.MaxPorts; PortNumber ++) {
    Offset = XHC_PORTSC_OFFSET + (0x10 * PortNumber);
    State  = XhcReadOpReg (Xhc, Offset);
    if (State & XHC_PORTSC_WRC) {
