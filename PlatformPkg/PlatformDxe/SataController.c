...
#define AHCI_MULTI_MAX_DEVICES              2    // multi port now is not supported, so give 2 for ide.
#define AHCI_MAX_PORTS                      32   // give max port count
  ...
  )
{
  ...
  for(Index=0;Index<HostInfo->HostCount;Index++){
    if(HostInfo->HostList[Index].HostType == PLATFORM_HOST_SATA){
      SataCtx = (PLATFORM_HOST_INFO_SATA_CTX*)(HostInfo->HostList[Index].HostCtx);
      if(CompareMem(HostInfo->HostList[Index].Dp, ParentDevicePath, SataCtx->DpSize) == 0){
        return EFI_SUCCESS;
      }
    }
  }

// add on ahci card without option rom.
  if(PciIo->RomImage == NULL && PciIo->RomSize == 0 && 
     ClassCode[1] == PCI_CLASS_MASS_STORAGE_SATADPA && 
     ClassCode[0] == PCI_IF_MASS_STORAGE_AHCI){
    return EFI_SUCCESS;
  }
