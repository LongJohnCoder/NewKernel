  ...
  )
{
  ...
  *Progress = Request;
  ...
  if ((Request != NULL) && !HiiIsConfigHdrMatch (Request, &gEfiFormsetGuidBoot, mBootMaintStorageName)) {
  ...
  if ((Request == NULL) || (StrStr (Request, L"OFFSET") == NULL)) {
    ...
    //
    ConfigRequestHdr = HiiConstructConfigHdr (&gEfiFormsetGuidBoot, mBootMaintStorageName, Private->BmmDriverHandle);
  ...
  )
{
  ...
  DEBUG((EFI_D_INFO, "BMCB A:%X Q:%X T:%X\n", Action, QuestionId, Type));

  if (QuestionId == KEY_VALUE_CSM_CONFIGURATION && Action == EFI_BROWSER_ACTION_CHANGED) {
    DEBUG((EFI_D_INFO, "CSM Key\n"));
    HiiGetBrowserData(
      &gPlatformSetupVariableGuid, 
      PLATFORM_SETUP_VARIABLE_NAME, 
      sizeof(SETUP_DATA), 
  ...
  HiiGetBrowserData(&gEfiFormsetGuidBoot, mBootMaintStorageName, sizeof(BMM_FAKE_NV_DATA), (UINT8 *) CurrentFakeNVMap);
  ...
  *ActionRequest = EFI_BROWSER_ACTION_REQUEST_NONE;
  ...
  if (FORM_SET_FD_ORDER_ID == Private->BmmPreviousPageId ||
      ...
      ((ROOT_FORM_ID == Private->BmmPreviousPageId ||
  ...
  //
  Status = HiiSetBrowserData (&gEfiFormsetGuidBoot, mBootMaintStorageName, sizeof (BMM_FAKE_NV_DATA), (UINT8 *) CurrentFakeNVMap, NULL);
  ...
  )
{
  ...
  BmmCallbackInfo->BmmPreviousPageId             = ROOT_FORM_ID;
  BmmCallbackInfo->BmmCurrentPageId              = ROOT_FORM_ID;
  BmmCallbackInfo->Notify.Notify                 = SetupItemBootNotify;
  ...
  BmmCallbackInfo->BmmSaveNotify.DriverHandle = BmmCallbackInfo->BmmSaveNotifyHandle;
...
// Init OpCode Handle and Allocate space for creation of Buffer
  ...
  DEBUG((EFI_D_INFO, "label: %X %X\n", FORM_BOOT_LEGACY_DEVICE_ID, LABEL_END));  
...
VOID InitTempBootOrder()
{
  ...
  if (VarData == NULL) {
    DEBUG((EFI_D_INFO, "Legacydevorder Not Present\n"));
  } else {
    Status = gRT->SetVariable (
  ...
  )
{
  ...
  UpdatePageId (CallbackData, ROOT_FORM_ID);
