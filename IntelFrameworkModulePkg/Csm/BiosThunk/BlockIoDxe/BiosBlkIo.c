...
typedef struct {
  ...
  UINT32  DeviceId;                 // A
  UINT16  MfgPointer;               // E
  UINT16  ProductNamePointer;       // 10
  UINT8   Class;                    // 12
  UINT8   SubClass;                 // 13
  UINT8   Interface;                // 14
  UINT8   DeviceIndicators;         // 15
  UINT16  Bcv;                      // 16
  UINT16  DisconnectVector;         // 18
  UINT16  Bev;                      // 1A
  UINT16  Reserved2;                // 1C
  UINT16  StaticResourceVector;     // 1E
...
VOID
GetDevPnpName(
  UINT8    *RomShadowAddress,
  UINT32   ShadowedRomSize,
  UINTN    DevPnpIndex,
  CHAR16   *PnpName,
  UINTN    PnpNameSize
  )
{
  EFI_LEGACY_EXPANSION_ROM_HEADER  *OpRomHdr;
  LEGACY_PNP_EXPANSION_HEADER      *PnpHdr;
  UINT16                           PnpOffset;
  UINTN                            PnpBcvIndex;  
  CHAR8                            *Str8 = NULL;
  CHAR8                            *DefName = "Unknown Storage";
  UINTN                            Index;
  UINTN                            Count;


  OpRomHdr = (EFI_LEGACY_EXPANSION_ROM_HEADER*)RomShadowAddress;  
  if(OpRomHdr->Signature != PCI_EXPANSION_ROM_HEADER_SIGNATURE){
    DEBUG((EFI_D_ERROR, "Bad OpRom Sign\n"));
    goto DefaultName;
  }

  PnpOffset = *(UINT16*)((UINTN)OpRomHdr + 0x1A);
  PnpBcvIndex  = 0;
  while(1){
    if(PnpOffset == 0){
      break;
    }
    PnpHdr = (LEGACY_PNP_EXPANSION_HEADER *)((UINTN)OpRomHdr + PnpOffset);
    if(PnpHdr->PnpSignature != SIGNATURE_32('$', 'P', 'n', 'P')){
      DEBUG((EFI_D_ERROR, "Bad PnpHdr Sign\n"));
      break;
    }

    if(PnpHdr->Bcv != 0){
      if(DevPnpIndex == PnpBcvIndex){
        if(PnpHdr->ProductNamePointer!=0){
          Str8 = (CHAR8*)((UINTN)OpRomHdr + PnpHdr->ProductNamePointer);
        } else {
          DEBUG((EFI_D_INFO, "ProductNamePointer:0\n"));
        }
        break;
      }
      PnpBcvIndex++;
    ...
    PnpOffset = PnpHdr->NextHeader;
    DEBUG((EFI_D_INFO, "PnpOffset:%X\n", PnpOffset));
  }

DefaultName:
  if(Str8 == NULL || Str8[0] == 0){
    Str8 = DefName;
  }

  Count = AsciiStrLen(Str8);
  if(Count > PnpNameSize/sizeof(CHAR16)-1-6){
    Count = PnpNameSize/sizeof(CHAR16)-1-6;
  }
  StrCpy(PnpName, L"SATA: ");
  
  for(Index=6;Index<Count+6;Index++){
    PnpName[Index] = Str8[Index-6];
    if(PnpName[Index] == 0){
      break;
    }
  }
  ...
  )
{
  ...
  *BbsIndex = 0xFFFF;
  ...
  for(Index=0;Index<BbsCount;Index++){
    ...
    if(BbsTable[Index].Bus != Bus || BbsTable[Index].Device != Dev ||
       BbsTable[Index].Function != Func){
      continue;
    }
    ...
    if(BcvIndex == HddIndex){
      *BbsIndex = (UINT16)Index;
      break;
    }
    HddIndex++;
  }
  ...
  for (Index = DiskStart; Index < DiskEnd; Index++) {
    ...
    if (BiosInitBlockIo (BiosBlockIoPrivate)) {
      ...
      GetDevPnpName(
        ...
        ShadowedRomSize, 
        Index-DiskStart, 
        BiosBlockIoPrivate->PnpName.Name,
        sizeof(BiosBlockIoPrivate->PnpName.Name)
        );
      FindDeviceBBSIndex(PciIo, LegacyBios, Index-DiskStart, &BiosBlockIoPrivate->PnpName.BBSIndex);
  
      DEBUG((EFI_D_INFO, "[%d] %s I:%d\n", Index, BiosBlockIoPrivate->PnpName.Name, BiosBlockIoPrivate->PnpName.BBSIndex));
