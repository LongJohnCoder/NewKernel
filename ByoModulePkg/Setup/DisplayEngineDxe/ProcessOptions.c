  ...
  )
{
  ...
  for (Index = Top; Index + 2 < Bottom; Index++, Count++) {
    ...
    if (GetStringWidth (String) / 2 > 1) {
      ClearWithBlt (Start, End, Index + 1, Index + 1, GetPopupColor ());
    }
    ...
    //
    if (String[0] == L' ') {
      ClearWithBlt (Start + 1, End - 1, Index + 1, Index + 1, GetPopupInverseColor ());
    }
  ...
  )
{
  ...
  UiConfirmDialog(DIALOG_INFO, NULL, NULL, TEXT_ALIGIN_CENTER, gEmptyString, &EventInfo->ErrorInfo, TimeOutString, NULL);
  ...
  )
{
  ...
  //
  UiConfirmDialog(DIALOG_INFO, NULL, NULL, TEXT_ALIGIN_CENTER, gEmptyString, gPassowordInvalid, NULL);
}
  ...
  *StringPtr = 0;
  ...
  if (Status == EFI_NOT_AVAILABLE_YET || Status == EFI_UNSUPPORTED) {
    ...
    if (Status == EFI_UNSUPPORTED) {
      UiConfirmDialog(DIALOG_INFO, NULL, NULL, TEXT_ALIGIN_CENTER, gEmptyString, gPasswordUnsupported, NULL);
    }
    FreePool (StringPtr);
    return EFI_SUCCESS;
  }
  ...
  if (StrCmp (StringPtr, TempString) == 0) {     
    ...
    //
    UiConfirmDialog(DIALOG_INFO, NULL, NULL, TEXT_ALIGIN_CENTER, gEmptyString, gConfirmError, NULL);
  ...
  *OptionString = NULL;
    ...
    if (Selected) {
      ...
      *OptionString = AllocateZeroPool (MaxLen * sizeof (CHAR16));
      ...
      for (Index = 0; Index < OrderList->MaxContainers; Index++) {
        ...
        if (OneOfOption == NULL) {
          DMSG ((EFI_D_ERROR, "\n ProcessOptions, EFI_IFR_ORDERED_LIST_OP, OneOfOption is NULL. \n"));
          if (SkipErrorValue) {
            //
          ...
          // Show error message
          //
          UiConfirmDialog(DIALOG_INFO, NULL, NULL, TEXT_ALIGIN_CENTER, gEmptyString, gOptionMismatch, NULL);
      ...
      while (!IsNull (&Question->OptionListHead, Link)) {
        ...
        if (!ValueInvalid) {
          ...
          // Show error message
          //
          DMSG ((EFI_D_ERROR, "\n ProcessOptions, EFI_IFR_ORDERED_LIST_OP, ValueInvalid. \n"));
          UiConfirmDialog(DIALOG_INFO, NULL, NULL, TEXT_ALIGIN_CENTER, gEmptyString, gOptionMismatch, NULL);
  ...
  case EFI_IFR_ONE_OF_OP:
    ...
    if (Selected) {
      ...
      *OptionString = AllocateZeroPool (BufferSize);
      ...
      if (OneOfOption == NULL) {
        DMSG ((EFI_D_ERROR, "\n ProcessOptions, EFI_IFR_ONE_OF_OP, Value :%d, OneOfOption is NULL. \n", QuestionValue->Value.u8));
        DMSG ((EFI_D_ERROR, "SkipErrorValue :%d. \n", SkipErrorValue));
        if (SkipErrorValue) {
          //
          // Not report error, just get the correct option string info.
          //          
  ...
  case EFI_IFR_STRING_OP:
    if (Selected) {
      ...
      Status = ReadString (MenuOption, gPromptForData, StringPtr, FALSE);
      if (EFI_ERROR (Status)) {
        FreePool (StringPtr);
        return Status;
      }
