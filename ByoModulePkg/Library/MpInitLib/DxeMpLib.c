  ...
  )
{
  ...
  if(CpuMpData->FixedApVector){
    if(CpuMpData->WakeupBuffer == (UINTN)-1){

      ApResetVectorSize = CpuMpData->AddressMap.RendezvousFunnelSize +
                          sizeof (MP_CPU_EXCHANGE_INFO);
      ASSERT(EFI_PAGES_TO_SIZE(EFI_SIZE_TO_PAGES(ApResetVectorSize)) <= CpuMpData->FixedApVectorSize);
      
      CpuMpData->WakeupBuffer      = CpuMpData->FixedApVector;
      ...
      DEBUG((EFI_D_INFO, "FixApVector Dxe Init\n"));
    }
    return;
  }
  ...
  if (CpuMpData->SaveRestoreFlag) {
    ...
    DEBUG((EFI_D_INFO, "ApResetVector:%X\n", (UINT32)StartAddress));
