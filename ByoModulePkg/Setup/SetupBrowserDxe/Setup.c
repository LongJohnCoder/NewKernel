...
BOOLEAN    gBeInteractiveStage = FALSE;
  ...
  )
{
  ...
  static EFI_HII_HANDLE    *NotFoundHandles = NULL;
  ...
  //
  // Initialize Not Found Handles.
  //
  for (Index = 0; HiiHandles[Index] != NULL; Index++);
  if (HandleCount != Index) {
    if (NotFoundHandles) {
      FreePool (NotFoundHandles);
      NotFoundHandles = NULL;
    }
    NotFoundHandles = HiiGetHiiHandles (NULL);
    for (Index = 0; HiiHandles[Index] != NULL; Index++) {
      NotFoundHandles[Index] = NULL;
    }
    HandleCount = Index;
  }
  ...
  for (Index = 0; HiiHandles[Index] != NULL; Index++) {
    ...
    if (NotFoundHandles[Index] == HiiHandles[Index]) {
      continue;
    }
    if (NotFoundHandles[Index] != NULL) {
      NotFoundHandles[Index] = NULL;
    }
    ...
    if (EFI_ERROR (Status) || IsListEmpty (&LocalFormSet->FormListHead)) {
      if (Status == EFI_NOT_FOUND) {
         NotFoundHandles[Index] = HiiHandles[Index];
      }
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "\n BrowserCallback, gBeInteractiveStage :%d, RetrieveData :%d, VariableName :%s. \n", gBeInteractiveStage, RetrieveData, VariableName));
  ...
  //
  // Load All FormSet to gather All Variable.
  //
  if (gBeInteractiveStage && RetrieveData ) {
    LoadAllHiiFormset ();
  }
    ...
    while (!IsNull (&gBrowserStorageList, Link)) {
      ...
      if (Storage->Type == EFI_HII_VARSTORE_NAME_VALUE ||
        ...
        if (Storage->HiiHandle != mSystemLevelFormSet->HiiHandle) {
          //DMSG ((EFI_D_ERROR, "BrowserCallback, Storage->HiiHandle :0x%x, mSystemLevelFormSet->HiiHandle :0x%x. \n",Storage->HiiHandle, mSystemLevelFormSet->HiiHandle));
          continue;
        }
      }

      Status = ProcessStorage (&TotalSize, &ResultsData, RetrieveData, Storage);
      DMSG ((EFI_D_ERROR, "BrowserCallback, ProcessStorage :%r. \n", Status));
      if (EFI_ERROR (Status)) {
        return Status;
      }
  ...
  )
{
  ...
  if (!IsListEmpty (&gBrowserSaveFailFormSetList)) {
    DMSG ((EFI_D_ERROR, "SubmitForForm, Process Failed Storge, \n"));
  ...
  )
{
  ...
  while (!IsNull (&gBrowserFormSetList, Link)) {
    ...
    if (LocalFormSet->FormSetTitle) {
      ...
      UiConfirmDialog(DIALOG_NO_REPAINT, gStrUiLoadFormset, NULL, TEXT_ALIGIN_CENTER, gEmptyString, FormSetTitle, NULL);  
      if (FormSetTitle) {
  ...
  )
{
  ...
  DMSG ((EFI_D_ERROR, "\n RegisterHotKey, Action :0x%x, KeyData :0x%x. \n", Action, KeyData->ScanCode));
  //
  ...
  DMSG ((EFI_D_ERROR, "RegisterHotKey, Copy KeyData :0x%x. \n", HotKey->KeyData->ScanCode));
  ...
  )
{
  ...
  if ((Action & BROWSER_ACTION_DISCARD) != 0) { 
    if ((Action & BROWSER_ACTION_EXIT) != 0) {
      CfmStr = gStrUiDiscardExit;
    } else {
      CfmStr = gStrUiDiscard;
    }
	
    if (SELECTION_YES == UiConfirmDialog(DIALOG_YESNO, gStrUiSetupConfirmation, NULL, TEXT_ALIGIN_CENTER, gEmptyString, CfmStr, NULL)) {
  ...
  if ((Action & BROWSER_ACTION_DEFAULT) != 0) {
    if (SELECTION_YES == UiConfirmDialog(DIALOG_YESNO, gStrUiSetupConfirmation, NULL, TEXT_ALIGIN_CENTER, gEmptyString, gStrUiLoadDefault, NULL)) {
  ...
  if ((Action & BROWSER_ACTION_SAVE_USER_DEFAULT) != 0) {
    if (SELECTION_YES == UiConfirmDialog(DIALOG_YESNO, gStrUiSetupConfirmation, NULL, TEXT_ALIGIN_CENTER, gEmptyString, gStrUiSaveUserDefault, NULL)) {
      Status = EFI_SUCCESS;
    } else {
      Status = EFI_NOT_READY;
    }
  ...
  if ((Action & BROWSER_ACTION_LOAD_USER_DEFAULT) != 0) {
    if (SELECTION_YES == UiConfirmDialog(DIALOG_YESNO, gStrUiSetupConfirmation, NULL, TEXT_ALIGIN_CENTER, gEmptyString, gStrUiLoadUserDefault, NULL)) {
      LoadAllHiiFormset ();
      Status = EFI_SUCCESS;
    } else {
      Status = EFI_NOT_READY;
...
{
  DMSG((EFI_D_ERROR, "FakeOemPlatformReset(), This is a EfiResetCold, please make new function in plafform setup.\n"));
