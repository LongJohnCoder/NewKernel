...
//MKE-20181112 Snoop On SMRR=WB, S3 hang becasue D0F2Rx9E[2]=0,Snoop Table Record SMMRAM,When Rx9E[2]=1,Wbinvd 
//Can't Flush Snoop Table,So need Flush SMMRAM before Rx9e[2] Chang 0 to 1._S
VOID
ClusterWbinvd (  
	  IN OUT VOID  *Buffer
)
...
VOID SnoopOnPatchFlushAllCache()
{
	EFI_STATUS Status;
	EFI_PEI_MP_SERVICES_PPI    *MpSvr;
	DEBUG((EFI_D_ERROR,"Mike SnoopOnPatchFlushAllCache\n"));
	Status = PeiServicesLocatePpi (
	...
	ASSERT_EFI_ERROR(Status);
	Status = MpSvr->StartupAllAPs((EFI_PEI_SERVICES **)GetPeiServicesTablePointer (), MpSvr, ClusterWbinvd, TRUE, 1000000, (EFI_PEI_SERVICES *)GetPeiServicesTablePointer ());
	AsmWbinvd();
}
//MKE-20181112_E
/**
  ...
  )
{
  ...
  if (GuidHob != NULL) {
    ...
    //MKE-2018112 Snoop On SMRR=WB, S3 hang becasue D0F2Rx9E[2]=0,Snoop Table Record SMMRAM,When Rx9E[2]=1,Wbinvd 
    //Can't Flush Snoop Table,So need Flush SMMRAM before Rx9e[2] Chang 0 to 1._S
    SnoopOnPatchFlushAllCache();
    //MKE-2018112_E
    Status = PeiServicesLocatePpi (
