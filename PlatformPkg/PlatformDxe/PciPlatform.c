...
#define IGD_TABLE_INDEX         0

STATIC PCI_OPTION_ROM_TABLE gOpRomTable[] = {
  {1, IGD_PCI_BUS, IGD_PCI_DEV, IGD_PCI_FUNC, 0xFFFF, 0xFFFF, IGD_VBIOS_FILE_GUID},
#if defined(HX001EM0_09)
  {1, 0xFFFF,      0xFF,        0xFF,         0x1A03, 0x2000, ASPEED_2400_VBIOS_FILE_GUID},
#endif
};
  ...
  )
{
  ...
  for(Index=0; Index<Count; Index++) {
    ...
    if(EFI_ERROR(Status)){
      ...
      HasFound = TRUE;
      DEBUG((EFI_D_INFO, "%a() %X%X OK\n", __FUNCTION__, DeviceId, VendorId));
      break;
    }
  }
...
// uefi   : 1
// no     : 2
  OpRomPost = PcdGet8(PcdVideoOpRomLaunchPolicy);
  if(OpRomPost != 0){
    gOpRomTable[IGD_TABLE_INDEX].Valid = 0;
  }
