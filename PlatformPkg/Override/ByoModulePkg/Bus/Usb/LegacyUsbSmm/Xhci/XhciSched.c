  ...
  )
{
  ...
  if (EFI_ERROR(Status)) {
    USB_DEBUG ((EFI_D_ERROR, "XhcDequeueTrbFromEndpoint: Stop Endpoint Failed, Status = %r\n", Status));
    goto Done;
  }
  ...
  if (Urb->Finished && Urb->Result == EFI_USB_NOERROR) {
    ...
    XHCI_DEBUG ((DEBUG_INFO, "XhcDequeueTrbFromEndpoint: Pending URB is finished: Length Actual/Expect = %d/%d!\n", Urb->Completed, Urb->DataLen));
  } else {
    Status = XhcSetTrDequeuePointer(Xhc, SlotId, Dci, Urb);
    if (EFI_ERROR (Status)) {
      XHCI_DEBUG ((DEBUG_ERROR, "XhcDequeueTrbFromEndpoint: Set Transfer Ring Dequeue Pointer Failed, Status = %r\n", Status));
      goto Done;
    }
  }
  ...
  )
{
  ...
  for (Index = 0; Index < Urb->TrbNum; Index++) {
    ...
    if (CheckedTrb->Type == TRB_TYPE_LINK) {
      ...
      CheckedTrb = (TRB_TEMPLATE *)(UINTN)PhyAddr;
  ...
  )
{
  ...
  EFI_LIST_FOR_EACH_SAFE (Entry, Next, &Xhc->AsyncIntTransfers) {
    ...
    if ((Urb->Ep.BusAddr == BusAddr) &&
      ...
      if (EFI_ERROR (Status)) {
        XHCI_DEBUG ((EFI_D_ERROR, "XhciDelAsyncIntTransfer: XhcDequeueTrbFromEndpoint failed\n"));
      }
  ...
  )
{
  ...
  EFI_LIST_FOR_EACH_SAFE (Entry, Next, &Xhc->AsyncIntTransfers) {
    ...
    Status = XhcDequeueTrbFromEndpoint (Xhc, Urb);
    if (EFI_ERROR (Status)) {
      XHCI_DEBUG ((EFI_D_ERROR, "XhciDelAllAsyncIntTransfers: XhcDequeueTrbFromEndpoint failed\n"));
    }
  ...
  )
{
  ...
  USB_DEBUG ((EFI_D_INFO, "Enable Slot Successfully, The Slot ID = 0x%x\n", EvtTrb->SlotId));
  ...
  )
{
  ...
  //
  USB_DEBUG ((EFI_D_INFO, "Disable device slot %d!\n", SlotId));
  ...
  )
{
  ...
  // Construct the disable slot command
  //
  USB_DEBUG ((EFI_D_INFO, "Disable device slot %d with cmd64!\n", SlotId));
