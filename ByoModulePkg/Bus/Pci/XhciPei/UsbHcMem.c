...
--*/
{
  ...
  for (Byte = 0, Bit = 0; Byte < Block->BitsLen;) {
    ...
    if (!USB_HC_BIT_IS_SET (Block->Bits[Byte], Bit)) {
      ...
      NEXT_BIT (Byte, Bit);
      if ((((UINTN)(Block->Buf + (Byte * 8 + Bit) * USBHC_MEM_UNIT))&0xFFF) == 0) {
         DEBUG ((EFI_D_INFO, "Encounter 4k boundary,at %x!\n",(Block->Buf + (Byte * 8 + Bit) * USBHC_MEM_UNIT)));
         NEXT_BIT (Byte, Bit);
         Available  = 0;
         StartByte  = Byte;
         StartBit   = Bit;
  ...
  if (Available < Units) {
    DEBUG ((EFI_D_INFO, "XHCI PEI Managed Memory exhausted!\n"));
    return NULL;
  }
...
//  Pool->PciIo   = PciIo;
  ...
  Pool->Head    = UsbHcAllocMemBlock (Xhc, Pool, USBHC_MEM_DEFAULT_PAGES);
...
--*/
{
  ...
  NewBlock = UsbHcAllocMemBlock (Xhc,Pool, Pages);
  ...
  )
{
  ...
  Mem = UsbHcAllocateMem (Xhc, Xhc->MemPool, AllocSize);
   if (Mem != NULL) {
    ZeroMem (Mem, Size);
  }
  return Mem;
