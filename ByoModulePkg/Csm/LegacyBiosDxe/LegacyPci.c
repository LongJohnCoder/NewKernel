...
// no     : 2
  ...
  if(OptionRomPost == 1 && ContainEfiImage(Rom, ImageSize)){    // uefi first && has uefi
    return FALSE;
  }
  ...
  )
{
  ...
  while (RomHeader.Generic->Signature == PCI_EXPANSION_ROM_HEADER_SIGNATURE) {
    ...
    if((RomHeader.Generic->PcirOffset & 3) !=0){
      DEBUG((EFI_D_ERROR, "PcirOffset is not 4 aligned\n"));
    }
  ...
  *ImageSize = Pcir->ImageLength * 512;
  ...
  if(*ImageSize > (UINTN)(RomHeader.PcAt->Size512 * 512)){
  ...
  )
{
  ...
  while (TRUE) {
    ...
    DescString = (CHAR8*)(UINTN)(((BbsTable[BbsIndex].DescStringSegment) << 4) + BbsTable[BbsIndex].DescStringOffset);
    if(DescString != NULL && DescString[0] == ' '){
      DescString++;
// [X][XXXX] -> X000:XXXX      
      BbsTable[BbsIndex].DescStringSegment = (UINT16)((((UINTN)DescString) >> 4) & 0xF000);
      BbsTable[BbsIndex].DescStringOffset  = (UINT16)(((UINTN)DescString) & 0xFFFF);
    }
  ...
  )
{
  ...
  *Flags = NO_ROM;
  ...
  if(!CheckLegacyRomPolicy(PciIo, LocalRomImage, LocalRomSize)){
    return EFI_UNSUPPORTED;
  }  
...
// some option rom's PCIR is not 4 bytes align, just warning it only.
    ...
    if((((PCI_EXPANSION_ROM_HEADER*)LocalRomImage)->PcirOffset & 3 ) != 0){
	  DEBUG((EFI_D_ERROR, "PCIR not dword aligned!\n"));
	}
