...
UINT32 GetMtcFromCmos(VOID)
{
  ...
  CmosOffset = PcdGet8(PcdMtcCmosOffset);
  
  OldTpl  = gBS->RaiseTPL(TPL_HIGH_LEVEL);  

  Mtc.Uint8.Data[0] = RtcRead(CmosOffset);
  Mtc.Uint8.Data[1] = RtcRead(CmosOffset+1);
  Mtc.Uint8.Data[2] = RtcRead(CmosOffset+2);
  Mtc.Uint8.Data[3] = RtcRead(CmosOffset+3); 

  gBS->RestoreTPL (OldTpl);  
  
  DEBUG((EFI_D_INFO, "MTC:%X\n", Mtc.Uint32));
  
  return Mtc.Uint32;
}

VOID SetMtcToCmos(UINT32 MyMtc)
{
  DATA32_4   Mtc;
  EFI_TPL    OldTpl;
  UINT8      CmosOffset;
  ...
  RtcWrite(CmosOffset,   Mtc.Uint8.Data[0]);
  RtcWrite(CmosOffset+1, Mtc.Uint8.Data[1]);
  RtcWrite(CmosOffset+2, Mtc.Uint8.Data[2]);
  RtcWrite(CmosOffset+3, Mtc.Uint8.Data[3]);
