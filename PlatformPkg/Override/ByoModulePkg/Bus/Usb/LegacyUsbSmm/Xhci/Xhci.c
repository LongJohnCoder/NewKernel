  ...
  )
{
  ...
  Index = 0;
  while ((State & XHC_PORTSC_PLS) == (BIT5|BIT6|BIT7) && Index < 5) {
    DEBUG (( EFI_D_INFO, "xhci PortNumber:%d State:%x\n",PortNumber, State));
	XhcSetRootHubPortFeature(This,PortNumber,EfiUsbPortReset);
	#if 0
    //
    // Mask off the port status change bits, these bits are write clean bit
    // 
    State &= ~ (BIT1 | BIT17 | BIT18 | BIT19 | BIT20 | BIT21 | BIT22 | BIT23);
    State |= XHC_PORTSC_RESET;
    XhcWriteOpReg (Xhc, Offset, State);
	#endif
    Status = XhcWaitOpRegBit(Xhc, Offset, XHC_PORTSC_PRC, TRUE, XHC_GENERIC_TIMEOUT);
    DEBUG ((EFI_D_INFO, "XhcUsbPortReset Status:%r\n",Status));

    Index ++;
    State = XhcReadOpReg (Xhc, Offset);
    DEBUG (( EFI_D_INFO, "xhci Index:%d State:%x\n",Index, State));
  }
ON_EXIT:
  return Status;
}
  ...
  )
{
  ...
  case EfiUsbPortReset:
	...
	State1=XhcReadOpReg (Xhc, Offset);
    if (State1&XHC_PORTSC_CCS)
    	{
    //
    ...
    XhcWaitOpRegBit(Xhc, Offset, XHC_PORTSC_PRC, TRUE, XHC_GENERIC_TIMEOUT);
		...
		{
    DEBUG ((EFI_D_INFO, "Skip XhcUsbPortReset!\n"));
	}
    break;
  ...
  )
{
  ...
  // Mask off the port status change bits, these bits are
  // write clean bit
  // BIT19 is XHC_PORTSC_WRC, we must clear it
  State  = XhcReadOpReg (Xhc, Offset);
  State &= ~ (BIT1 | BIT17 | BIT18 | BIT20 | BIT21 | BIT22 | BIT23);
  ...
  case EfiUsbPortConnectChange:
    ...
    //
    State |= XHC_PORTSC_CSC | XHC_PORTSC_PLC | XHC_PORTSC_WRC | XHC_PORTSC_CEC;
    XhcWriteOpReg (Xhc, Offset, State);
    // check if it has been cleared
    for (Index = 0; Index < 100; Index ++) {
      State  = XhcReadOpReg (Xhc, Offset);
      if ((State & (XHC_PORTSC_WRC | XHC_PORTSC_CSC)) == 0) {
        break;
      }
      DEBUG ((EFI_D_INFO, "re-clear port change State[%x]:%x\n",Index, State));
      XhcWriteOpReg (Xhc, Offset, State);
    }
  ...
  )
{
  ...
  if (Urb == NULL) {
    XHCI_DEBUG ((DEBUG_ERROR, "XhcTransfer[Type=%d]: failed to create URB!\n", Type));
  ...
  if (Status == EFI_TIMEOUT) {
    ...
    if (RecoveryStatus == EFI_ALREADY_STARTED) {
      ...
      Status = EFI_SUCCESS;
      USB_DEBUG ((DEBUG_ERROR, "XhcTransfer[Type=%d]: pending URB is finished, Length = %d.\n", Type, Urb->Completed));
  ...
  *DataLength     = Urb->Completed;
  ...
  if ((*TransferResult == EFI_USB_ERR_STALL) || (*TransferResult == EFI_USB_ERR_BABBLE)) {
    ...
    if (EFI_ERROR (RecoveryStatus)) {
      USB_DEBUG ((DEBUG_ERROR, "XhcTransfer[Type=%d]: XhcRecoverHaltedEndpoint failed!\n", Type));
    }
  }
...
ON_EXIT:
  ...
  if (EFI_ERROR (Status)) {
    USB_DEBUG ((EFI_D_ERROR, "XhcControl: error - %r, transfer - %x\n", Status, *TransferResult));
  }
...
ON_EXIT:
  ...
  if (EFI_ERROR (Status)) {
    USB_DEBUG ((EFI_D_ERROR, "XhcBulk: error - %r, transfer - %x\n", Status, *TransferResult));
  }
...
)
{
  ...
  XhcWriteOpReg (Xhc, XHC_USBSTS_OFFSET, XHC_USBSTS_EINT);    // Set BIT3 to clear it
