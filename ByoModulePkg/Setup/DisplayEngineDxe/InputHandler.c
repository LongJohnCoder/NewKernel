  ...
  )
{
  ...
  do {
    ...
    case CHAR_CARRIAGE_RETURN:
      StringWidth = GetStringWidth (StringPtr);
      if ((StringWidth >= ((Minimum + 1) * sizeof (CHAR16))) ||
        ...
        //
        UiConfirmDialog(DIALOG_INFO, NULL, NULL, TEXT_ALIGIN_CENTER, &NullCharacter, gMiniString, NULL);
  ...
  )
{
  ...
  if (Question->OpCode->OpCode == EFI_IFR_DATE_OP) {
    ...
    case 2:
      Minimum = PcdGet16(PcdMinimalValidYear);
      Maximum = PcdGet16(PcdMaximalValidYear);
      EraseLen = 5;
...
//-   DEBUG((EFI_D_INFO, "IsDate:%d, EditValue:%d, Minimum:%d, Maximum:%d\n", IsDate, EditValue, Minimum, Maximum));
      if(IsDate){
  ...
  )
{
  ...
  Top    = gStatementDimensions.TopRow + 1;
  Bottom = gStatementDimensions.BottomRow - 1;
  if (PopUpWidth < gOptionBlockWidth) {
    Start  = gStatementDimensions.LeftColumn + gPromptBlockWidth + 1;
    End    = Start + PopUpWidth + POPUP_FRAME_WIDTH;
  }
  ...
  LastTopOptionIndex = TopOptionIndex;

  if (MenuLinesInView >= PopUpMenuLines + 2) {
    if (MenuOption->Row < (Top + Bottom)/2) {
      //
      //Show pop-up at top of screen.
      //
      if (MenuOption->Row + PopUpMenuLines + 2 < Bottom) {
        Top = MenuOption->Row + 1;
        Bottom = Top + PopUpMenuLines + 1;            
      } else {
        //
        //Show pop-up in the middle.
        //
      ...
      //
      if (MenuOption->Row - PopUpMenuLines - 2 > Top) {
        Bottom = MenuOption->Row - 1;
        Top = Bottom - PopUpMenuLines - 1;                  
      } else {
        //
        //Show pop-up in the middle.
        //
    ...
    if (PopUpMenuLines == HighlightOptionIndex + 1) {
      ShowDownArrow = FALSE;
    } else {
      ShowDownArrow = TRUE;
    }
  ...
  do {
    ...
    //
    if (LastTopOptionIndex != TopOptionIndex) {
      LastTopOptionIndex = TopOptionIndex;
      bRepaintFrame = TRUE;
    }
    if (bRepaintFrame) {
    ClearWithBlt (Start, End, Top, Bottom, GetPopupColor ());
    }
    ...
    for (Index = TopOptionIndex; (Index < PopUpMenuLines) && (Index2 < Bottom); Index++) {
      ...
      if (Index == HighlightOptionIndex) {
          ...
          gST->ConOut->SetAttribute (gST->ConOut, GetPopupColor ());
          PrintStringAtWithWidth (Start + 2 + StringWidth, Index2, L"", PopUpWidth - POPUP_PAD_SPACE_COUNT-StringWidth);		  
        } else {
          gST->ConOut->SetAttribute (gST->ConOut, GetPopupColor ());
          PrintStringAt (Start + 2, Index2, StringPtr);
          PrintStringAtWithWidth (Start + 2 + StringWidth, Index2, L"", PopUpWidth - POPUP_PAD_SPACE_COUNT - StringWidth);
    ...
    if (USER_INPUT_MOUSE == InputType) {
      ...
      if ((Mouse.Column < (Start + 2)) || (Mouse.Column > (End - 2)) || (Mouse.Row < (Top + 1)) || (Mouse.Row > (Bottom - 1))) {
        ...
        if (MOUSE_LEFT_CLICK == Mouse.Action) {
          if (HighlightOptionIndex == TopOptionIndex + (Mouse.Row - Top - 1)) {
            Key.UnicodeChar = CHAR_CARRIAGE_RETURN;            
            Key.ScanCode = SCAN_NULL;            
          } else {
            HighlightOptionIndex = TopOptionIndex + (Mouse.Row - Top - 1);
            continue;
          }
        } 
